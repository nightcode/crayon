    ...
    function chart(selection) {
      selection.each(function() {
        ...
        if (g.empty()) {
          ...
          barPaint.x(function(d, i, layer) {
            return xScale(d.x) + layer * xWidthScale.rangeBand();
          }).y(function() {
            return height;
          }).height(function(d) {
            return yScale(d.y);
          }).width(function() {
            return xWidthScale.rangeBand();
          });
        }
        ...
        g.select(".y.bar-axis").call(yAxis);
        var layers = g.select(".bars-chart").selectAll(".bar-chart").data(filteredSeries.length > 0 ? d3.layout.stack()(filteredSeries) : []), layersEnter = layers.enter().append("path").attr("class", "bar-chart"), layersExit = layers.exit().remove(), layersUpdate = layers.style("fill", function(d, i) {
          return colors(i);
        }).attr("d", barPaint);
      });
    }
    var width = 400, height = 200, xScale = d3.scale.ordinal(), yScale = d3.scale.linear(), xAxis = crayon.axis().scale(xScale).orient("horizontal"), yAxis = crayon.axis().scale(yScale).orient("vertical"), barPaint = crayon.bar(), series, seriesNames, colors, xTickFormat = function(d) {
      return d;
    };
