(function() {
  ...
  crayon.tooltip = function() {
    function tooltip(x, y, width, height) {
      var path = [], xDirection, yDirection;
      switch (orient) {
       case "ne":
        {
          xDirection = 1;
      ...
      path.push("M", x, ",", y, "L", x + dx * xDirection, ",", y - dy * yDirection, "L", x - (leftPart * width + margin) * xDirection, ",", y - dy * yDirection, "L", x - (leftPart * width + margin) * xDirection, ",", y - (dy + height + 2 * margin) * yDirection, "L", x + (rightPart * width + margin) * xDirection, ",", y - (dy + height + 2 * margin) * yDirection, "L", x + (rightPart * width + margin) * xDirection, ",", y - dy * yDirection, "L", x + dx * 2 * xDirection, ",", y - dy * yDirection, "Z");
      return path.join("");
    }
    var orient = "ne", dx = 12, dy = 12, leftPart = .3, rightPart = .7, margin = 7;
  ...
  crayon_barChart = function() {
    function chart(selection) {
      selection.each(function() {
        ...
        var layers = barsChart.selectAll(".bar-chart").data(filteredSeries.length > 0 ? d3.layout.stack()(filteredSeries) : []);
        ...
        layers.style("fill", function(d, i) {
          ...
          barsChart.append("g").attr("class", "tooltip").attr("transform", function() {
            return "translate(" + i * xWidthScale.rangeBand() + ",0)";
          });
        }, true);
        layers.exit().remove();
        ...
        bars.enter().append("rect").on("mouseout", function() {
          barsChart.selectAll(".tooltip").remove();
        });
        bars.attr("x", function(d) {
          ...
          var x = xScale(d.x), y = yScale(d.y);
          var gTooltip = barsChart.selectAll(".tooltip"), tooltipPath = gTooltip.append("path");
          var xValue = gTooltip.append("text").attr().text(d.x), yValue = gTooltip.append("text").attr().text(d.y);
          var xSvgRect = xValue.node().getExtentOfChar(0), ySvgRect = yValue.node().getExtentOfChar(0);
          var w = d3.max([ xValue.node().getComputedTextLength(), yValue.node().getComputedTextLength() ]), h = xSvgRect.height + ySvgRect.height;
          var tooltip = crayon.tooltip();
          var xDirection = x + w + tooltip.margin() * 2 > width ? "w" : "e", yDirection = y - h - tooltip.margin() * 2 > 0 ? "n" : "s";
          if (xDirection === "e") {
          ...
          xValue.attr("x", tooltipRect.w).attr("y", tooltipRect.n + xSvgRect.height);
          yValue.attr("x", tooltipRect.w).attr("y", tooltipRect.n + xSvgRect.height * 2);
        });
        bars.exit().remove();
      });
    }
